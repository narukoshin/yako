...
var bip39Wordlist = [2048]string{
	...
	"plate", "play", "player", "pleasure", "plenty", "plot", "plunge", "poem", "poet", "point",
	...
	"sunset", "super", "supply", "support", "suppose", "sure", "surface", "surgery", "surprise", "surround",
...
func wordIndex(word string) (int, bool) {
	...
	for lo < hi {
		mid := (lo + hi) / 2
		if bip39Wordlist[mid] < word {
			lo = mid + 1
		} else if bip39Wordlist[mid] > word {
			hi = mid
		} else {
			return mid, true
		}
...
func GenerateRecoveryPhrase() (string, error) {
	...
	for i := range phraseWords {
		...
		val := (uint16(buffer[byteStart]) << 8) | uint16(buffer[byteStart+1])
		val = (val >> (8 - bitOffset)) & 0x7FF
		words[i] = bip39Wordlist[val]
	}
...
func PhraseToEntropy(phrase string) ([]byte, error) {
	...
	for i, idx := range indices {
		...
		bitOffset := bitStart % 8
		val := idx << (8 - bitOffset)
		buffer[byteStart] |= byte(val >> 8)
		buffer[byteStart+1] |= byte(val)
	}

	entropy := buffer[:recoveryEntropyBytes]
	checksum := buffer[recoveryEntropyBytes] >> (8 - checksumBits)
